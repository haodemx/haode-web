...
function card(r){{return `<article class='card'><div class='media'>${{img(r)}}<span class='badge'>${{esc(r.normalization_status)}}</span></div><div class='body'><h2>${{esc(r.product_name)}}</h2><p>${{esc(r.sku)}} · ${{esc(r.category)}}</p><p>${{esc(r.qc_status)}} · ${{esc(r.real_asset_status)}}</p></div></article>`}}
